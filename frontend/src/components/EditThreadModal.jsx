...
import { 
  ...
  Settings, ChevronUp, Paperclip, Eye, Send, Save
} from 'lucide-react';
import { useModal } from '../context/ModalContext';
import { usePosts } from '../context/PostsContext';
import './CreateThreadModal.css'; // Reuse CSS for layout

const EditThreadModal = () => {
  const { isEditThreadOpen, closeEditThread, postToEdit } = useModal();
  ...
  useEffect(() => {
    if (isEditThreadOpen && postToEdit) {
      setTitle(postToEdit.title || '');
      setPostContent(postToEdit.snippet || '');
      setSelectedCategory(postToEdit.category || '');
      setMediaPreviews(postToEdit.images || []);
      
      // Auto-resize logic for pre-filled text
      setTimeout(() => {
  ...
  if (!isEditThreadOpen) return null;

  return (
    <div className="modal-overlay" onClick={closeEditThread}>
      <div className="modal-container" onClick={e => { e.stopPropagation(); setIsCategoryOpen(false); }}>
        ...
        <div className="modal-header">
          <div className="modal-header-left">
            ...
            <div>
              <h2 className="modal-title">Edit Thread</h2>
              <p className="modal-subtitle">Make changes to your existing post.</p>
            </div>
          </div>
          <button className="modal-close-btn" onClick={closeEditThread}>
            <X size={20} />
          </button>
        </div>
            ...
            <div className="x-editor-container">
              <div className="x-editor-left">
                <div className="x-avatar">
                  <img 
                    src={user?.profileImage || postToEdit?.avatar || "https://www.gravatar.com/avatar/00000000000000000000000000000000?d=mp&f=y"} 
                    alt="Profile" 
                  />
                </div>
              </div>
        ...
        <div className="modal-footer">
          <div className="footer-right-actions" style={{marginLeft: 'auto'}}>
            <button className="btn-secondary" onClick={closeEditThread}>
              Cancel
            </button>
